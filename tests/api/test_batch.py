...
class TestBatchProvision:
    def _setup_range(self, client):
        ...
        tmpl = client.post("/templates", json={
            "name": "batch-tmpl", "version": "1.0", "yaml": "assets: []", "is_public": True
        }).json()
        r = client.post("/ranges", json={
            "name": "batch-range-1", "template_id": tmpl["id"]
        }).json()
        return r["id"]
    ...
    def test_batch_provision_multiple(self, client):
        ...
        tmpl = client.post("/templates", json={
            "name": "multi-tmpl", "version": "1.0", "yaml": "assets: []", "is_public": True
        }).json()
        rids = []
        for i in range(3):
            r = client.post("/ranges", json={
                "name": f"multi-range-{i}", "template_id": tmpl["id"]
            }).json()
            rids.append(r["id"])
...
class TestRangeStats:
    ...
    def test_stats_after_creation(self, client):
        ...
        tmpl = client.post("/templates", json={
            "name": "stats-tmpl", "version": "1.0", "yaml": "assets: []", "is_public": True
        }).json()
        for i in range(2):
            client.post("/ranges", json={
                "name": f"stats-range-{i}", "template_id": tmpl["id"]
            })
        resp = client.get("/ranges/stats")
        assert resp.status_code == 200
        data = resp.json()
