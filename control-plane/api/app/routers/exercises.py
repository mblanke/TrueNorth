...
async def ai_enhance_aar(
    ...
    ai_orchestrator_url = os.getenv("AI_ORCHESTRATOR_URL", "http://ai-orchestrator:8000")

    try:
        import httpx

        prompt = _build_aar_prompt(report_data)
        async with httpx.AsyncClient(timeout=60.0) as client:
            resp = await client.post(
                f"{ai_orchestrator_url}/generate",
                json={
                    "task_type": "aar-analysis",
                    "prompt": prompt,
                    "max_tokens": 2000,
                    "temperature": 0.3,
                },
        ...
        ai_analysis = ai_result.get("text", ai_result.get("response", ""))
        ...
        ai_html = (
            ...
            f'<p class="ai-meta">Generated by {ai_result.get("model", "AI")}</p>'
            f"</div>"
        )
        if aar.report_html:
