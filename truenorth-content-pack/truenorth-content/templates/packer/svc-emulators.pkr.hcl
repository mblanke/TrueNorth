# Golden image svc-emulators — DERIVED (no ISO). Built by cloning + provisioning a base template.
# Catalogue: base_iso=derived from ubuntu-lts; sensor_baked=yes; packer_status=todo(vsphere).
# NOTE: Red Analyst internet-service emulation. No distinct ISO.
...
variable "base_template_svc-emulators" { type = string, default = "ubuntu-lts" }   # base golden template to clone from
