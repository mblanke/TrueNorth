# Golden image precomp-host — DERIVED (no ISO). Built by cloning + provisioning a base template.
# Catalogue: base_iso=derived from win10-22h2 / srv2019 snapshot; sensor_baked=yes; packer_status=derived.
# NOTE: Staged artifacts for log/IR analysis start. Snapshot of a base Windows image - no ISO.
...
variable "base_template_precomp-host" { type = string, default = "win10-22h2" }   # base golden template to clone from
