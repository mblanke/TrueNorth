# Golden image win11-24h2 — vSphere-iso build -> vCenter Content Library.
# Catalogue: os=windows 11 24H2 (current-estate victim); base_iso=Win11_Enterprise_Eval_x64.iso; sensor_baked=yes.
# NOTE: Pairs with EDR-present scenarios. No packer file yet.
...
source "vsphere-iso" "win11-24h2" {
  ...
  iso_paths = ["[${var.vsphere_datastore}] Win11_Enterprise_Eval_x64.iso"]
...
build {
  sources = ["source.vsphere-iso.win11-24h2"]
