# Golden image remnux — DERIVED (no ISO). Built by cloning + provisioning a base template.
# Catalogue: base_iso=derived from ubuntu-lts (REMnux installer); sensor_baked=no; packer_status=todo(vsphere).
# NOTE: ALRA static/dynamic. Built on ubuntu base - no distinct ISO.
...
variable "base_template_remnux" { type = string, default = "ubuntu-lts" }   # base golden template to clone from
...
source "vsphere-clone" "remnux" {
  ...
  template            = var.base_template_remnux
  vm_name             = "remnux"
  linked_clone        = false   # full snapshot of the base, then provision on top
}
build {
  sources = ["source.vsphere-clone.remnux"]
