# Golden image usersim — DERIVED (no ISO). Built by cloning + provisioning a base template.
# Catalogue: base_iso=derived from ubuntu-lts; sensor_baked=no; packer_status=todo(vsphere).
# NOTE: THE NOISE FLOOR - generates false positives / benign traffic. No distinct ISO.
...
variable "base_template_usersim" { type = string, default = "ubuntu-lts" }   # base golden template to clone from
...
source "vsphere-clone" "usersim" {
  ...
  template            = var.base_template_usersim
  vm_name             = "usersim"
  linked_clone        = false   # full snapshot of the base, then provision on top
}
build {
  sources = ["source.vsphere-clone.usersim"]
