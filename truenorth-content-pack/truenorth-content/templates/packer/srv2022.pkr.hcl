# Golden image srv2022 — vSphere-iso build -> vCenter Content Library.
# Catalogue: os=windows Server 2022 (modern DC/member); base_iso=SERVER_EVAL_x64FRE_en-us.iso; sensor_baked=yes.
# NOTE: Current estate. vSphere packer present.
...
source "vsphere-iso" "srv2022" {
  ...
  iso_paths = ["[${var.vsphere_datastore}] SERVER_EVAL_x64FRE_en-us.iso"]
...
build {
  sources = ["source.vsphere-iso.srv2022"]
