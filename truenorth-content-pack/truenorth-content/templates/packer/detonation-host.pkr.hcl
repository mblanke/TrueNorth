# Golden image detonation-host — DERIVED (no ISO). Built by cloning + provisioning a base template.
# Catalogue: base_iso=derived from win10-22h2; sensor_baked=no; packer_status=derived.
# NOTE: Isolated dynamic analysis (ALRA) - no egress ever.
...
variable "base_template_detonation-host" { type = string, default = "win10-22h2" }   # base golden template to clone from
