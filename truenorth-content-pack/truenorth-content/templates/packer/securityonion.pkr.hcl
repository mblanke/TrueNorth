# Golden image securityonion — vSphere-iso build -> vCenter Content Library.
# Catalogue: os=linux 2.4 (sensor + telemetry appliance); base_iso=securityonion-2.4.10-20240220.iso; sensor_baked=n/a.
# NOTE: Baked into most range templates - OpenSearch feed. Sensor appliance.
...
source "vsphere-iso" "securityonion" {
  ...
  iso_paths = ["[${var.vsphere_datastore}] securityonion-2.4.10-20240220.iso"]
...
build {
  sources = ["source.vsphere-iso.securityonion"]
