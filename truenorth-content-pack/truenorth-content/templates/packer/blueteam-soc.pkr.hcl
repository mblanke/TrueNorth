# Golden image blueteam-soc — DERIVED (no ISO). Built by cloning + provisioning a base template.
# Catalogue: base_iso=derived (multi-service); sensor_baked=yes; packer_status=todo(vsphere).
# NOTE: Live Red-v-Blue MSEL (TEMP64 PO_004). Composite stack.
...
variable "base_template_blueteam-soc" { type = string, default = "derived" }   # base golden template to clone from

source "vsphere-clone" "blueteam-soc" {
  ...
  template            = var.base_template_blueteam-soc
  vm_name             = "blueteam-soc"
  linked_clone        = false   # full snapshot of the base, then provision on top
}
build {
  sources = ["source.vsphere-clone.blueteam-soc"]
